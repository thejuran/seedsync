...
must_haves:
  truths:
    ...
    - "Dropdown menu fades in/out with ~150ms animation on open/close"
    ...
    - "Menu items have subtle hover transition (~100ms)"
  artifacts:
...
No SCSS compilation errors related to undefined variables.
  ...
  <done>
`_bootstrap-overrides.scss` contains dark dropdown theme CSS variable overrides using app color variables. Dropdown fade animation (150ms) and item hover transition (100ms) are implemented. File compiles successfully.
  </done>
</task>
...
6. **Fade animation verification:**
   ```bash
   grep -n "opacity\|transition.*0.15s" src/angular/src/app/common/_bootstrap-overrides.scss
   ```
   Shows dropdown fade animation CSS.
</verification>
...
- [ ] Dropdown fade animation (~150ms) implemented
- [ ] Menu item hover transition (~100ms) implemented
