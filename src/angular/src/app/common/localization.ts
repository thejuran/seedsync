export class Localization {
  ...
  static Notification = class {
    ...
    public static readonly CONFIG_VALUE_BLANK = (section: string, option: string) =>
      `Setting ${section}.${option} cannot be blank.`;
  ...
  static Modal = class {
    ...
    public static readonly DELETE_LOCAL_MESSAGE = (name: string) =>
      `Are you sure you want to delete <b>${name}</b> from the local server?`;

    public static readonly DELETE_REMOTE_TITLE = 'Delete Remote File';
    public static readonly DELETE_REMOTE_MESSAGE = (name: string) =>
      `Are you sure you want to delete <b>${name}</b> from the remote server?`;
  };
