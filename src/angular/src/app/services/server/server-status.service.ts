...
import { ServerStatus, ServerStatusJson, serverStatusFromJson } from './server-status';
import { BaseStreamService } from '../base/base-stream.service';

@Injectable({ providedIn: 'root' })
export class ServerStatusService extends BaseStreamService {
  private _status: BehaviorSubject<ServerStatus> = new BehaviorSubject(
    new ServerStatus({
      server: {
        up: false,
  ...
  private parseStatus(data: string) {
    ...
    const status = serverStatusFromJson(statusJson);
