...
import { ModelFile, modelFileFromJson } from './model-file';
...
export class ModelFileService extends BaseStreamService {
  ...
  private _files: BehaviorSubject<Immutable.Map<string, ModelFile>> = new BehaviorSubject(
    Immutable.Map<string, ModelFile>()
  );
  ...
  private parseEvent(name: string, data: string) {
    if (name === this.EVENT_INIT) {
      ...
      for (const file of parsed) {
        newFiles.push(modelFileFromJson(file));
      }
      t1 = performance.now();
